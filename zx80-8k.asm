...
; ==============================================================
; An Assembly Listing of the Operating System of the ZX80 8K ROM
; ==============================================================
; -------------------------
; Last updated: 17-APR-2014
; -------------------------
;
; Work in progress. poglad did it based on the work of gwearmouth.
; This file will cross-assemble a recreated version of the "missing"
; ZX80 8K ROM.  The file can be modified to change the behaviour of
; the ROM when used in emulators although there is no spare space
; available.
;
...
;; START
L0000:  LD      BC,$7FFF        ; Set BC to the top of possible RAM.
...
;; DISPLAY-4
L02A9:  LD      A,$F4           ; (7)  load A with first R delay value
...
; --------------------------
;   This subroutine is called to generate the central TV picture. The R
;	register is incremented, with each instruction, to $FF by the time
;	it completes, and an interrupt will occur as soon as the Program
;	Counter reaches the HALT.  (24 clock cycles)

;; DISPLAY-5
L02B5:  LD      R,A             ; (9) Load R from A.          R = $FC
        LD      A,$DD           ; (7) load future R value.        $FD

        EI                      ; (4) Enable Interrupts           $FE

        JP      (HL)            ; (4) jump to the echo display.   $FF
...
; at all. The TV picture is formed by the external video hardware.
		...
		LD		HL,$405A		; Location before the first program line
								; - 16474 decimal.
        LD      ($4059),HL      ; store in DATADD
		INC		HL				; next location is first after system variables
...
L1AC5:  CALL    L0EA7           ; routine FIND-INT to fetch the
        ...
        LD      HL,STACK_HL     ; address: STACK-HL is
